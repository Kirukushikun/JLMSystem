...
**Roles are enforced by `role:` middleware** (`CheckRole`), checked per-route in `routes/web.php`. A user can hold more than one role — roles live in a `role_user` pivot table, checked via `User::hasRole()`/`hasAnyRole()`; the `users.role` column is only a legacy "primary role" pointer (used for the post-login landing page). `requestor` is the public-facing role (submit + track own requests); `division_head`/`reviewer`/`vp`/`purchasing` are the approval pipeline (in that order); `admin` can do everything. A requestor's `company`/`dept` live on the `users` table and pre-fill the Submit form, but both are freely editable — a requestor can pick a different farm or department if a particular request is for one; nothing is server-enforced back to the account's value.
